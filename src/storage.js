...
const DEFAULT_PREFERENCES = {
    ...
    imageQuality: 'medium', // Renamed from selectedImageQuality to match common usage if needed, or keep as is. Sticking to simple keys.
    advancedMode: false,
...
function getConfigDir() {
    ...
    if (platform === 'win32') {
        configDir = path.join(os.homedir(), 'AppData', 'Roaming', 'half-life-config'); // Renamed to half-life-config to match project name
    } else if (platform === 'darwin') {
        configDir = path.join(os.homedir(), 'Library', 'Application Support', 'half-life-config');
    } else {
        configDir = path.join(os.homedir(), '.config', 'half-life-config');
    }
